...
export default function Page() {
  ...
  const [videosData, setVideoData] = useState<any[]>([]);

  console.log(courseData,videosData )
  ...
  useEffect(() => {
    ...
    if (user?.primaryEmailAddress?.emailAddress) {
        getVideos(user.primaryEmailAddress?.emailAddress).then((data) => {
           setVideoData(data)
          });
      ...
      fetchData(user.primaryEmailAddress?.emailAddress).then((data) => {
        const courseData = data.find(
          (item: any) => item.courseItems[0].title == title
        );
    ...
    <section className="relative">
      ...
      <div className="relative max-w-6xl mx-auto px-4 sm:px-6">
        <div className="py-12 md:py-20">
          ...
          {/* Items */}
          {(videosData.length && courseData) ? <div className="max-w-sm mx-auto grid gap-10 md:grid-cols-2 lg:grid-cols-1 items-start md:max-w-2xl lg:max-w-none">
          {courseData.video_id.map((id: any) => {
              const item = videosData.find(item => item.videoId == id)
  ...
  return (
    <div className="relative flex gap-4 justify-between p-6 bg-white rounded-lg shadow-xl">
        ...
        <div>
          ...
          <div className="w-full">
            <Link
              ...
              href={{
                pathname: "/courses/videos",
                query: { videoId: nftData.videoId },
              }}
            >
              Play
            </Link>
      ...
      <div className="flex-shrink-0 mr-5">
        <PieChartWithCenterLabel completed={0} remaining={nftData.total_duration
} />
      </div>
    </div>
  );
